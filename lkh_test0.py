...
def get_df_evol(err_dic, path_all, gen, chk):
    bin_maf = prm.BIN_MAF

    df_maf = alltls.get_maf(path_all)

    for k_set in err_dic.keys():
        list_maf = alltls.compute_maf_evol(k_set, gtgl=gen, chk_sz=chk)
...
def plot_test(df_maf, col_set):
    ...
    plt.suptitle("")
    plt.savefig('maf_evol.gtgl.pooled.chunk{}.png'.format(df_maf.shape[0]),
...
if __name__ == '__main__':
    ...
    for p_set in params:
        ...
        POOL = 'IMP.chr20.pooled.beagle2.{}.chunk{}.corr.vcf.gz'.format(gtgl, sz)
    ...
    print('dfplot\n', df_plot)
    plot_test(df_plot, col_set=['gt', 'gl'])