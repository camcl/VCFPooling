...
v1.2.1: implement multiprocessing
'''
...
candidates = alltls.extract_variant_onfreq(os.path.join(path_gt_files,
                                                        raw['ref'].replace('.gl', '.gt')),
                                           [0.495, 0.505])
snp_to_rm: List[str] = candidates['id'].str.strip(' ').str.slice(3).values
idv_to_keep: List[str] = []
print('Number of SNPsto analyze separately: ', len(snp_to_rm))  # 225 with [0.49, 0.51], 121 with [0.495, 0.505]
