...
def get_pop():
    df_pop = pd.read_csv(os.path.join(prm.WD, '20130606_sample_info.csv'),
                      sep=',',
                      header=0,
                      usecols=[0, 1])
...
def make_index(raw_data, src=None, id='id'):
    if src is None:
        src = prm.SOURCE
    group = get_pop()
...
if __name__ == '__main__':
    ...
    ### MAF BEHAVIOR
    alleles_plots.plot_maf_evol(set_errors, ALL)
