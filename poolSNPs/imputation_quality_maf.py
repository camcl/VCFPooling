...
tabbeaglegl.plot.scatter('maf_info', 'precision_score', ax=axes[0, 0], s=0.7)
axes[0, 0].set_ylim(0.0, 1.0)
tabbeaglegl.plot.scatter('maf_info', 'accuracy_score', ax=axes[0, 1], s=0.7)
axes[0, 1].set_ylim(0.0, 1.0)
tabbeaglegl.plot.scatter('maf_info', 'concordance', ax=axes[0, 2], s=0.7)
axes[0, 2].set_ylim(0.0, 1.0)
tabbeaglegl.plot.scatter('maf_info', 'f1_score', ax=axes[0, 3], s=0.7)
axes[0, 3].set_ylim(0.0, 1.0)
tabbeaglegl.plot.scatter('maf_info', 'r_squared', ax=axes[1, 0], s=0.7)
axes[1, 0].set_ylim(-0.2, 1.0)
if entro is not None:
    tabbeaglegl.plot.scatter('maf_info', 'cross_entropy', ax=axes[1, 1], s=0.7)
...
for ax in axes.flatten()[:-2]:
    ...
    ax.set_xlabel('true minor allele frequency', color='k')
